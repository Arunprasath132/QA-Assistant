...
export default function TestCaseGenerator() {
  ...
  const generate = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API_BASE}/testcases/generate`, { ...form, test_types: selectedTypes });
  ...
  const exportFile = async (format) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/testcases/export/${format}`, testCases, { responseType: 'blob' });
  ...
  const handleUpload = async (file) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/testcases/upload`, formData);
      setTestCases(res.data.test_cases);
